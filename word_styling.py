...
def export_doc(template_path, output_path, daten):
    ...
    # Absätze: alle Runs durchgehen
    for p in doc.paragraphs:
        for run in p.runs:
            run.text = ersetze(run.text)

    # Tabellenzellen: ebenfalls alle Runs durchgehen
    for t in doc.tables:
        for row in t.rows:
    ...
    for para in doc.paragraphs:
        if para.text.strip().startswith("Versicherungsangebot für "):
            ...
            # Unterstreichung unten beibehalten
            p = para._element
