...
def export_doc(template_path, output_path, daten):
    ...
    from docx.shared import Inches  # Falls noch nicht oben vorhanden

for table in doc.tables:
    table.alignment = WD_TABLE_ALIGNMENT.CENTER
    table.autofit = False  # Sehr wichtig: automatisches Anpassen deaktivieren

    if len(table.columns) >= 3:
        table.columns[0].width = Inches(2.8)  # Produkt
        table.columns[1].width = Inches(1.6)  # mit SB
        table.columns[2].width = Inches(1.6)  # ohne SB

    for row_index, row in enumerate(table.rows):
        for col_index, cell in enumerate(row.cells):
            cell.width = table.columns[col_index].width  # explizit Breite setzen
