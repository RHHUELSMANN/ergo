...
if st.button("Prämie berechnen"):
    if reisepreis and alter_input and reise_von and reise_bis:
        ...
        rrv_ev_mit_df = blatt["rrv-ev-mit"][blatt["rrv-ev-mit"]["Reisepreis bis"] >= reisepreis]
        rrv_ev_mit = first_hit(rrv_ev_mit_df) if not rrv_ev_mit_df.empty else None
        rrv_ev_ohne_df = blatt["rrv-ev-ohne"][(blatt["rrv-ev-ohne"]["Altersgruppe"] == a_gruppe) & (blatt["rrv-ev-ohne"]["Reisepreis bis"] >= reisepreis)]
        rrv_ev_ohne = first_hit(rrv_ev_ohne_df) if not rrv_ev_ohne_df.empty else None
        ...
        rows = [
            ("Reiserücktritt Einmal",
             (rrv_ev_mit["Prämie"], rrv_ev_mit["Tarifcode"]) if rrv_ev_mit is not None else ("–", "–"),
             (rrv_ev_ohne["Prämie"], rrv_ev_ohne["Tarifcode"]) if rrv_ev_ohne is not None else ("–", "–")),
        ...
        rrv_ev_mit_betrag = rrv_ev_mit["Prämie"] * reisepreis if rrv_ev_mit["Prämie"] < 1 else rrv_ev_mit["Prämie"] if rrv_ev_mit is not None else 0
        rrv_ev_ohne_betrag = rrv_ev_ohne["Prämie"] * reisepreis if rrv_ev_ohne["Prämie"] < 1 else rrv_ev_ohne["Prämie"] if rrv_ev_ohne is not None else 0
        rus_ev_mit_betrag = rus_ev_mit * reisepreis if rus_ev_mit < 1 else rus_ev_mit
        rus_ev_ohne_betrag = rus_ev_ohne * reisepreis if rus_ev_ohne < 1 else rus_ev_ohne
        ...
        st.session_state["word_data"] = {
            ...
            "rrv_ev_mit": rrv_ev_mit_betrag, "rrv_ev_ohne": rrv_ev_ohne_betrag,
            ...
            "rus_ev_mit": rus_ev_mit_betrag, "rus_ev_ohne": rus_ev_ohne_betrag,
