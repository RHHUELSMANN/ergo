...
with st.form("eingabeformular"):
    ...
    # Initiales Alter-Feld (kann manuell überschrieben werden)
    ...
    for idx, col in enumerate(cols, start=1):
        with col:
            geb_eingaben.append(
                st.text_input("", key=f"gb{idx}", label_visibility="collapsed", placeholder=f"Geb. {idx}")
            )
    ...
    if geburts_alter:
        ...
        # Überschreibe alter_text nur, wenn Geburtstage gültig
...
if submit:
    ...
    # Beispiel-Tariftabelle (hier kannst du deine Excel-Logik reinladen)
    df = pd.DataFrame([
        ["Reiserücktritt", "Einmal", "123,45 €", "234,56 €"],
        ["", "Jahres", "345,67 €", "456,78 €"],
        ["", "Sparfuchs", "567,89 €", "678,90 €"],
    ...
    # GPT-Antwort nach der Tabelle
    if frage.strip():
        absatzliste = lade_absätze_aus_pdf("ergo_tarife.pdf")
        antwort, fundstellen = frage_an_gpt(frage, absatzliste)
